...
def ai_play(lvlNumber, bot, maxIter, delayTime=250):
	state = readLevel(lvlNumber)
	screen = GameScreen(state, lvlNumber)
...
if __name__ == '__main__':
	...
	parser.add_argument('-l','--level', action='store', dest='levelNo', default='0', type=str, help='Which level to play (1-488) or [random] for a randomly selected level')
	...
	if (args.levelNo == 'random'):
		randLevels = []
		with open("rand_solve_levels.txt") as rsl:
			randLevels = rsl.readlines()
		args.levelNo = random.choice(randLevels)
